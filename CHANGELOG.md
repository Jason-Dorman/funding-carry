...
- **The fill model is four knobs and two refusals.** Latency (jittered over half to one and a half of `SIM_LATENCY_MS`, seeded so an ordinary run replays), slippage always applied *against* the order and **capped at its limit**, orders above `SIM_PARTIAL_THRESHOLD` printing in `SIM_PARTIAL_SLICES` whole-contract slices that sum to the order exactly, and non-crossing orders **resting** until the market reaches them. It refuses an order when the newest snapshot is absent or older than `SIM_BOOK_MAX_AGE_SECS` — a simulator that kept filling against a frozen book would look entirely healthy doing it — and it refuses a fractional contract count, which is the quantization rule enforced at the venue boundary.
- **One goroutine owns the book**, fed over unbuffered channels from quickfixgo's connection goroutines. Unbuffered because a buffer would mean a message was accepted by a channel rather than by the engine: once the engine has stopped, `select` picks at random between queueing an order and reporting the shutdown, so half the time an order is acknowledged into a buffer nothing will ever read.
- **Fills are recorded before they are reported.** A report sent but not recorded is a fill the system cannot account for; one recorded but not sent is recovered by the FIX resend. ExecIDs carry the process start, because `fills` is keyed on `(venue, venue_exec_id)` with `ON CONFLICT DO NOTHING` and a counter restarting from one each boot would make a second run's fills vanish into the first run's rows. `fee` stays NULL: the simulator observes no fee tier, and a fabricated one would flow straight into the P&L decomposition.
- **`fix_sessions` now upserts** ([api-spec §5.3](docs/api-spec.md#53-constraints-and-indexes)), one row per process run: opened at the first logon, rewritten at each logout with the sequence numbers the store reached and a rising disconnect count, reopened when the client returns. The first live run wrote a row per *logon* instead and produced three rows reading 1, 2, 1 — fixed in the same session.

New config: `FIX_STORE_PATH` (a Compose volume — `ResetOnLogon` is off, so the sequence store has to outlive the container), `SIM_PARTIAL_SLICES`, `SIM_BOOK_POLL_SECS`, `SIM_BOOK_MAX_AGE_SECS`. New metrics: `fix_session_up`, `fix_msgs_total`, `fix_resend_events_total` — unprefixed, because carry's initiator will export the same three and the `session` label is what separates them — plus `simv_orders_total{result}`, `simv_book_age_seconds` and `simv_resting_orders`.

**Demonstrated against the live stack** with a scripted FIX client: `D → 8(NEW) → 8(FILLED)` filling at 2400.48 against a 2400.00 ask (two basis points, redoable by hand from the stored row); a 35-contract order printing 11 + 11 + 13 with CumQty monotonic; a cancel before any fill; a fractional order refused by name; and, after `docker compose restart sim-venue`, a session resuming at sequence **23** rather than at 1. A client that reconnected with a lost store triggered a full SequenceReset-GapFill resend of the venue's history — the behaviour that is the reason this is a FIX session and not a socket with JSON on it.
