...
- **Parts 16, 17 and 18 stop and ask before a line of code is written.** These are the only parts that can move real money — 16 places perp orders, 17 signs on-chain swaps, 18 runs the first fully automated carry. When Jason asks to build one, **say so in the conversation first**: name what has to be funded or provisioned, say plainly that this part spends real money, and wait for explicit confirmation *in that conversation* before starting. Do not begin and mention it afterwards; do not treat a general "do Part 16" as the confirmation. The prerequisites and what goes live are tabulated under [Funding and live-capital gates](docs/build-plan.md#-funding-and-live-capital-gates) — but the table is not the mechanism, this rule is, because a file nobody opens is not a gate. **Nothing before Part 16 can move money**: the only order-placing code is the FIX initiator, and both binaries refuse to start unless its counterparty is the simulator (`FIX_TARGET` must be `SIMV`; there is no live FIX endpoint in v1), no Ethereum signing code exists in the repo, and the Coinbase credential is view-only.
