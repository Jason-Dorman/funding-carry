...
type ExecReport struct {
    ...
    At         time.Time       // the VENUE's time for the event; on a resend, the original event's
    ReceivedAt time.Time       // when the report reached THIS process, our clock, stamped at the boundary
}
```

**Two times, because they answer two questions.** `At` is the venue's — on a resent report it is the *original* event's time, which is what the round trip and the `fills` row need. `ReceivedAt` is stamped once by the `Venue` implementation the moment the report arrives in this process and is never taken off the wire. `ORDER_TIMEOUT` runs on our clock and waits for a report to reach us, so the distribution that validates it has to be measured between two instants on that clock — the `Ack` returned from `Submit`, and `ReceivedAt`. `At` would exclude inbound network and parse time, which is precisely the tail the deadline exists to catch, and on a resend it can be hours stale. A venue that leaves `ReceivedAt` zero contributes no acknowledgement latency rather than a wrong one. *(PO revision to [CHANGE-002](CHANGE-002-ack-latency-histogram.md).)*
...
**Per-order timeout bounds the acknowledgement, not the fill.** Each tracked order carries a deadline, `Ack.At + ORDER_TIMEOUT` (§7). An order the venue has **not acknowledged** by then is *overdue*; an acknowledged order is never overdue however long it rests, because a resting limit order past a deadline is the order working, and whether to keep it working is the router's call rather than a clock's. On the second leg of an entry, an overdue order is the trigger for unwinding the first ([architecture §6.4](architecture.md#64-live-carry-entry-week-5)). The tracker answers `Overdue(now)` and refreshes `carry_orders_open` / `carry_orders_overdue` from `Observe(now)`, which the binary calls every second and once more when it stops, so the last scrape of a shutdown is a coherent pair.

**How the timeout gets settled.** The two gauges show whether `ORDER_TIMEOUT` is being missed; they cannot produce the value that would replace it. Prometheus scrapes every 15 s ([deploy/prometheus/prometheus.yml](../deploy/prometheus/prometheus.yml)), an acknowledgement arriving just past a 30 s deadline leaves the gauge raised for a fraction of that, and a gauge carries a count and no latency — an ack at 31 s and an ack at 45 s are the same observation. The distribution is therefore measured directly, by **`carry_order_ack_seconds`** (§6): from `Ack.At` to the `ReceivedAt` of the report that takes the order out of `PENDING`, which is the same edge, and the same clock, the deadline judges on.

**The two series overlap, deliberately, and the overlap is the interesting part.** The histogram holds every order that *eventually* got a response, at its true latency. The counter holds every order whose response *missed the deadline* — including the ones that later arrived. So a late answer is in **both**, and an order never answered at all is in the counter **only**; a synthetic observation is never written for a timeout, because that would corrupt the one distribution the value is set from. They are not a partition: **dividing the counter into the histogram's count double-counts the late answers.** The timeout rate is the counter against orders *submitted*, not against the histogram. An adopted order is in neither, having no acknowledgement to measure. **30 s is retained** until paper trading produces data; the value is revisited then. *(Gauge limitation found by the Part 8 follow-up adversarial review; instrument added by [CHANGE-002](CHANGE-002-ack-latency-histogram.md).)* What to do about an overdue order is the router's and the risk engine's. *(PO decision after the Part 8 review; the first draft measured submit-to-terminal.)*

**Retention.** Orders are kept for the life of the process; there is no way to drop one — with one exception, which is what makes the rule safe. Adoption has to create a record before the rule table can judge the report that caused it, so a **report that is not applied undoes its own adoption** and the record goes away again. Without that, one malformed report would leave a `PENDING` order no report can ever move and nothing can remove: permanently open, permanently overdue, and a standing instruction to Part 15's router to unwind the other leg of an entry. *(Found by the Part 8 follow-up adversarial review.)* Everything else is kept. A resend can replay an order's reports long after it ended, and a forgotten order would be adopted back as a new one — the duplicate the tracker exists to recognise. At a handful of orders a day this is a few hundred bytes each, which is not memory under pressure. *(PO decision after the Part 8 review, to be revisited only if the order rate changes by orders of magnitude.)*

**An `invalid` report is an invariant violation, not a warning.** A venue whose reports disagree with themselves — more filled than was ordered, a cancel losing a fill — means the system's belief about its own position is in doubt, which is the one thing a delta-neutral book cannot carry. The PO decided (after the Part 8 review) that this halts the venue: Part 13 wires `invalid` to a `risk_events` row and refuses submission on that venue until a manual reset, alongside the other hard stops. Until Part 13 the tracker logs it at Error and counts it, and does nothing else — the loudest thing it can say without a risk engine to say it to.
...
| `carry_order_ack_seconds` | histogram | venue, leg | **time to the venue's first response**, despite the name: the report that takes the order out of `PENDING`, whether that is a `NEW`, a fill, or a **rejection** — all three are the venue answering and all three stop the order being overdue, and measuring only a `NEW` would drop the orders answered fastest out of the distribution (§3.2). Measured from `Ack.At` to the report's `ReceivedAt`, both on **our** clock, because that is the wait `ORDER_TIMEOUT` races. Buckets `0.05 0.1 0.25 0.5 1 2 5 10 30 60`: fine where responses land, coarse out to and past the deadline, so an observation at or beyond 30 s is a response that came back after it. **This is the series `ORDER_TIMEOUT` is set from** ([CHANGE-002](CHANGE-002-ack-latency-histogram.md)). An adopted order contributes nothing — no acknowledgement, no interval. Not pre-created at zero: `leg="spot"` cannot happen in v1 and a zero would advertise a path the system lacks |
| `carry_order_ack_timeouts_total` | counter | venue, leg | orders that passed `ORDER_TIMEOUT` with no response, **counted once each** (the edge is detected by `Observe`; nothing in this system fires a timeout). It **overlaps** the histogram rather than complementing it: an order answered late is counted here *and* observed there at its true latency, and one never answered is counted only here. **Do not divide this into the histogram's count** — that double-counts the late answers; the timeout rate is this against orders submitted |
| `carry_orders_open` | gauge | venue | orders handed to the venue and not yet ended; refreshed on every report, every second, and once more as the process stops. Overdue orders are a subset of these, so `carry_orders_overdue > carry_orders_open` in one scrape is a bug in the exporter, not a state of the book |
| `carry_orders_overdue` | gauge | venue | orders the venue has not acknowledged within `ORDER_TIMEOUT` (§3.2). A venue that has stopped answering shows as this rising and staying. It **shows that the timeout is being missed but cannot yield the value to replace it** — at a 15 s scrape a marginally late acknowledgement is usually invisible, and a gauge carries no latency (§3.2) |
