...
**Version:** 3.4 (supersedes `hyperliquid_signal_app_system_spec.md`, `hyperliquid_updated_system_spec.md`, `hyperliquid_formulas_thresholds.md`, `perps-carry-mvp-spec.md`, and `SPEC.md` — the last of which described a Rust / offshore-venue / bridge design that no longer applies; anything still valid from it is folded into §4)
...
- **v3.4 (2026-09-16)** — three open items added (§12 items 7, 8 and 9) rather than any requirement changed: where the live system runs and what must be up while a position is open, whether item 4's "self-recorded" is the requirement it meant, and which wallet `WALLET_ADDRESS` names once Part 17 introduces a second one. Both follow from naming the actual deployment — Docker Desktop on a development laptop — which the spec had never stated and [architecture §7.1](architecture.md#71-historical-recovery--the-backfill-pattern) had contradicted.
...
- **Margin health is read, not derived — and "read" means the venue's reported buffer, not a ratio we divide.** *(Amended v3.3, 2026-09-22.)* This bullet previously said the venue "returns `margin_ratio = available_margin / liquidation_threshold`". It does not: it returns the two operands and the developer guide states the formula, which `internal/coinbase` then computes — so the principle was being stated and broken in the same sentence. Coinbase also publishes **two opposite-direction figures both called "margin ratio"** (the divided one, higher-is-safer, fatal at 1.0; and the app widget's percentage, higher-is-more-dangerous, fatal at 100%), and this system had named its column, metric and alert after the one the PO does not see. The venue reports margin health *directly* as `liquidation_buffer_percentage`, fatal at 0%, and **that is the risk engine's input** ([ADR-0022](decisions/0022-margin-health-from-venue-buffer.md)); the divided ratio is retained as a reconciliation cross-check and is never a fallback control value. Both definitions and the full field inventory live in [venue doc §4.1](venue-coinbase-perps.md#41-margin-ratio-names-two-opposite-quantities--read-this-before-using-either) and nowhere else.
...
| Margin health | `liquidation_buffer_percentage`, **reported** by the venue; **0% = liquidation** *(amended v3.3, 2026-09-22 — was `available_margin / liquidation_threshold`, which is derived rather than read, and points the opposite way to the Coinbase UI's figure of the same name; [ADR-0022](decisions/0022-margin-health-from-venue-buffer.md))* | floor from config (`LIQUIDATION_BUFFER_FLOOR_PCT`); block if breached at proposed size. Missing or stale → block entries, `MARGIN_DATA_UNAVAILABLE`, **never** fall back to the derived ratio |
| Margin ratio *(derived)* | `available_margin / liquidation_threshold` | reconciliation cross-check only — not a control value |
| Funding-cost cap | funding paid < 30% of expected profit (only relevant on reverse carry / directional) | — |

**Carry decision rules (v1):**
- ENTER: `funding > 0` AND `funding_z ≥ z_enter` AND `expected_carry ≥ k × costs` AND `basis > 0` (future rich) AND `basis < basis_extreme` AND margin health OK *(reported buffer present, fresh, above floor)* AND at least one whole contract affordable AND not inside the maintenance window AND not BLOCKED.
- HOLD: position on, funding still > 0, margin health OK, basis not extreme against.
...
| 2 | FIX loop | ✅ **done** 2026-09-17 — Parts 7–8. `sim-venue` (Part 7, 2026-09-16) answers NewOrderSingle and OrderCancelRequest and prices fills against recorded top-of-book; `carry`'s initiator (Part 8) sent an order over its own session and read the fill back, and a fill that printed while `carry` was down arrived by resend recovery on restart, both against the live stack. The venue state cache is Part 9 | Initiator sends NOS to sim-venue, receives ExecutionReport, sequence numbers survive restart. | Manual carry #2 closed. |
...
5. The `TODO(verify)` items in [venue-coinbase-perps.md](venue-coinbase-perps.md). **Closed 2026-08-24:** official funding-rate exposure (published, at `future_product_details.funding_rate`) and overnight margin per contract (`{long 0.24525, short 0.33475}`, from the public product payload). **Still open:** fee tier, settlement times, fractional-contract rejection — all of which need an account or a live order — and, added 2026-09-22, the four **margin-health fields the balance summary returns and nothing parses** plus the scale of `liquidation_buffer_percentage`, which needs an account *holding a position* and is owned by [build plan Part 5A](build-plan.md#part-5a--balance-summary-field-completion).
6. Rust port: revisit only if job targets narrow to prop-shop trading systems.
7. **Where the live system runs, and what has to be up while a position is open.** *(Raised 2026-09-16.)* The plan sequences the risk engine before live capital (§10 wk 4 → wk 5) but never says where any of it executes. v1 runs under Docker Desktop on a development laptop, which cannot be continuously available — and a carry is held for ≥24h across several funding hours by design (§1 wallet track), so an open position necessarily outlives the session that opened it.

   The exposure is specific rather than general. The book is delta-neutral in aggregate, but the **margin sits at Coinbase while the offsetting gain sits in a wallet on Base**: a sharp rally drains perp margin at a venue that cannot see the on-chain leg backing it, and the hedging gain is not movable in minutes. That is the transition [§6.8](#68-treasury) and Part 18 exist for, and it is unattended whenever the laptop is off.

   What must be continuously available is therefore the **risk engine, hard stops and kill switch** (Part 13) and something able to act on them (Part 15) — *not* the recorder. A recorder that is down costs history, and the [backfill pattern](architecture.md#71-historical-recovery--the-backfill-pattern) says exactly how much. A risk engine that is down costs money.

   Three ways to cover it, and they are not exclusive:
   - **Rest the stop at the venue.** A stop or reduce-only order held by Coinbase fires whether or not this system is alive, and is the only mitigation that does not depend on uptime at all. [api-spec §1](api-spec.md) records that only market and limit are confirmed for US derivatives, with other order types `[verify in Part 16]` — **that verification is now a decision input rather than a detail**, because if stops rest at the venue the always-on requirement softens considerably.
   - **A small always-on host.** The Compose stack runs unchanged; only its location differs. The Base leg's session key is already router-scoped, allowance-capped and expiry-bound (§9), so a compromised host has a blast radius that was bounded for other reasons and pays off here.
   - **Hold only while attended.** Rejected on its face: funding accrues hourly and the strategy requires holding across many hours.

   Not urgent at `MAX_NOTIONAL_USD` 500 and ≤ 3×, and **nothing about it blocks Parts 7–15**, none of which touch live capital. It must be settled before Part 16, which is the first part that can open a position this system would have to watch. Decide with the Part 16 order-type verification in hand.
8. **Whether §12 item 4's "self-recorded" is the requirement it meant.** *(Raised 2026-09-16, follows from 7.)* The z-score is computed on funding, and the funding series is derivable from candles back to the contract's launch — so the 30-day threshold does **not** depend on uptime, and the 2026-09-23 date is reachable regardless of how intermittently the stack runs. What genuinely cannot survive downtime is Tier 2 microstructure (`cb_book_snapshots` above all), and it is an open question whether Tier 2 is used as a rolling window at all or only as current state at decision time — Part 11 settles that. The word "self-recorded" was chosen for [ADR-0015](decisions/0015-backfilled-funding-provenance.md)'s reason: backfilled hours are built from one-minute candle closes rather than three-minute trade VWAPs. There is now a measurement of what that costs — the live estimator sat +0.22 pp annualised from the venue's published rate, sd 1.91 pp, over 30 hours — which was not available when the criterion was written.
9. **Which wallet `WALLET_ADDRESS` names, once there are two.** *(Raised 2026-09-16.)* §1's doctrine says "one named wallet", and today that is unambiguous: an EOA, reachable by ENS, holding ETH and USDC on Base. Part 17 makes it ambiguous. [api-spec §2.2](api-spec.md#22-write-path-basevenue-week-5) swaps **"from the smart wallet"** with a session key **"provisioned by hand from the owner wallet"** — an ERC-4337 smart account has its own address, distinct from the EOA that controls it, and the config has one variable. It does not matter for Part 6, which only reads balances. It starts mattering at Part 17, when the ETH the strategy holds moves into the smart account, and at Part 18, whose treasury reconciliation would otherwise balance against an address that is not holding the position. Either `WALLET_ADDRESS` changes meaning, or the config grows a second variable and `base_state` grows a way to say which address a row describes — the same provenance problem as `spot_px_source`, one layer up. **Decide with Part 17, not before.**
