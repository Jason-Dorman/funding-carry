...
| 2 | FIX loop | 🟡 **partial** — the acceptor half is done (Part 7, 2026-09-16): `sim-venue` answers NewOrderSingle and OrderCancelRequest, prices fills against recorded top-of-book, and its sequence numbers survived a container restart, demonstrated with a scripted client. The initiator in `carry` is Part 8 and the venue state cache Part 9 | Initiator sends NOS to sim-venue, receives ExecutionReport, sequence numbers survive restart. | Manual carry #2 closed. |
