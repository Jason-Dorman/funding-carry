...
1. **Acceptance is demonstrated, not asserted.** Each part's summary shows the command run and the observed evidence (test output, induced alert, DB rows). "It should work" is not done.
   **A performance demonstration also states its window, and the window spans at least one cycle of every periodic process involved** ([engineering principles](engineering-principles.md#code-review-checklist)). Part 9 measured a refresh budget over five minutes against a one-hour connection lifetime and recorded the criterion as met; it was false on the cold path the whole time, and the window made it unfalsifiable rather than wrong. Compress the cycle if waiting it out is impractical.
...
- **The suite creates and drops its own database** (`carry_integration`) beside the one `TEST_DATABASE_URL` points at, and never writes to that one. A package other than `internal/db` that needs a seeded schema uses `internal/db/dbtest` (Part 9): `Open(t, name)` builds a throwaway database of its own name under the same rules — no `DATABASE_URL` is a failure, never a skip — and `Seed` writes fixtures through the real writer, so a fixture reaches its table by the path production rows do. Each package names its own database because `go test ./...` runs packages in parallel. Self-recorded market history is the system's primary asset ([ADR-0002](decisions/0002-one-timescaledb-instance.md)); a test run must not be able to delete weeks of it. It connects over the published host port `15432`, which is deliberately not `5432` so a Postgres already on the developer's machine cannot be mistaken for the stack's.
- FIX integration: real quickfixgo initiator ↔ acceptor over localhost, including the restart/resend scenario (Part 8 acceptance) — stop the initiator with an order resting, fill it while nobody is listening, restart on the same store, assert the fill arrives by resend. Both ends run in-process against fakes for the market and the writer, so this lives in the ordinary `internal/fix` suite rather than behind the `integration` tag; the tag is for the schema.
