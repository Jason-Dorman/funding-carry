...
**Last verified:** 2026-08-16 from the sources listed in §9. Anything marked `TODO(verify)` was not confirmable from official docs at that date and must be checked against the live API in week 1 before it is relied on.
...
- **Publication:** real-time and projected funding are published via CDE **FIX and SBE** market data (institutional). Historical funding is "available upon request." Coinbase's derivatives market-data page advertises funding-rate data. **`TODO(verify)` whether the retail Advanced Trade REST/WS exposes a funding-rate field for `ETP-20DEC30-CDE`** (the INTX perps expose `future_product_details.perpetual_details.funding_rate`; the CFM product may not).
...
Auth: CDP API key + JWT (`Authorization: Bearer <token>`); same for REST and WS. Base URL `https://api.coinbase.com/api/v3/brokerage/`.

| Need | Endpoint / channel | Notes |
|---|---|---|
| Product metadata | `GET products` filtered `product_type=FUTURE`, `contract_expiry_type=PERPETUAL`, venue `FCM`; `GET products/{product_id}` | Public variants exist without auth. Confirm `ETP-20DEC30-CDE` appears and read `future_product_details` (contract size, expiry, venue). |
| Candles, ticker, book, trades | `products/{id}/candles`, `best_bid_ask`, `product_book`, `market_trades` (public variants available) | Used for futures mark computation and Tier-2 microstructure. |
| Spot reference | same endpoints on `ETH-USD` | Used for spot mark. |
| Funding rate (official) | `TODO(verify)` — check `future_product_details` and any perpetual-details block for CFM products | If absent, compute (see §3). |
...
| WebSocket | market channels: `ticker`, `level2`, `market_trades`, `candles`, `status`; user channels: `user`, `futures_balance_summary` | Official Python SDK documents `futures_balance_summary` as a WS user channel; confirm the same channels accept `ETP-20DEC30-CDE`. |
...
6. Record: overnight margin per contract, current fee, and whether the API returns an official funding rate. Fill in the `TODO(verify)` items in this file and bump "last verified."
...
| Funding data | public WS/REST | FIX/SBE institutional; retail REST `TODO(verify)`; compute locally |
